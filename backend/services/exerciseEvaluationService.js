...
// Comprehensive Assessment History Hubs — reconstructs the same rich
// answer-sheet shape submitExerciseAttempt returns live, but for an
// ALREADY-SUBMITTED attempt, for the mentee/mentor history views.
//
// exercise_attempt_answers only ever persisted one row per TOP-LEVEL
// question (student_answer, is_correct, score_awarded, feedback) — it
// never stored comprehension's per-sub-question breakdown, since that
// was only ever built in-memory for the live report card. So: the
// top-level isCorrect/scoreAwarded/feedback below always come from the
// persisted row (frozen at submit time, authoritative, never
// recomputed) — but comprehension's `subResults` breakdown is
// regenerated by re-grading the stored student_answer against the
// question's CURRENT content_payload/grading_rubric, the only way to
// get that detail without a schema migration to persist it.
//
// Disclosed tradeoff: if a mentor edits a comprehension exercise's
// answer key after mentees have already attempted it, an old attempt's
// sub-question breakdown will reflect the edited key even though its
// stored top-level score does not change to match. Re-authoring an
// exercise after live attempts exist is already a known gap elsewhere
// in this codebase (see the Hierarchical Content Tree plan doc); this
// is the same class of issue, now also visible here.
async function buildAttemptAnswerSheet(attemptId, exerciseId) {
  const [questions, storedAnswers] = await Promise.all([
    ExerciseQuestion.findAll({
      where: { exercise_id: exerciseId },
      order: [["order_index", "ASC"]],
    }),
    ExerciseAttemptAnswer.findAll({
      where: { exercise_attempt_id: attemptId },
    }),
  ]);

  const questionsById = new Map(questions.map((question) => [question.id, question]));

  return storedAnswers.map((answerRow) => {
    const question = questionsById.get(answerRow.question_id);

    // Question was deleted after the attempt was submitted — show what
    // the persisted answer row still has rather than dropping the row.
    if (!question) {
      return {
        ...
        contentPayload: null,
        gradingRubric: null,
        points: Number(answerRow.score_awarded) || 0,
        studentAnswer: answerRow.student_answer,
        isCorrect: answerRow.is_correct,
        scoreAwarded: Number(answerRow.score_awarded),
    ...
    let subResults;
    if (question.question_type === QUESTION_TYPES.COMPREHENSION) {
      subResults = gradeComprehension(question, answerRow.student_answer).subResults;
    }
...
// Normalizes an ExerciseAttempt row into the same camelCase shape
// submitExerciseAttempt's live response returns, so ReportCard.jsx
// (frontend) never needs a second field-mapping path for historical
// attempts.
function formatAttemptSummary(attempt, passingPercentage) {
  return {
    id: attempt.id,
